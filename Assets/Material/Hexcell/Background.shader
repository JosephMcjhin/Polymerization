Shader "Custom/Background"
{
    Properties{
        ...
        [ShowAsVector2]_Bias("Bias",Vector)=(0,0,0,0)
    }
    SubShader{
        ...
        Pass{
            ...
            trans_vert vert(raw_vert v){
                ...
                o.main_uv.xy = (v.texcoord_main * _MainTex_ST.xy + _MainTex_ST.zw)/2 + fixed2(0.25,0.25); //注意这里和正常思维是相反的。
                return o;
            }
            //使用插值后的顶点信息对像素进行赋值。
            fixed4 frag(trans_vert i):SV_Target{
                fixed4 tex_color = tex2D(_MainTex, i.main_uv + _Bias.xy) * _Color;
                return tex_color;
            }
            ENDCG
